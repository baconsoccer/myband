...
function get_articles() {
    ...
    $stmt->bind_result($title) or die ('Error binding results results 1');
...
function get_some_articles() {
    ...
    $stmt = $mysqli->prepare($query) or die ('Error preparing mailadres');
    $stmt->bind_result($title, $content, $imagelink) or die ('Error binding results');
    $stmt->execute() or die ('Error executing mailadres');
    $results = array();
    while ($stmt->fetch()) {
        $article = array();
        $article[] = $title;
